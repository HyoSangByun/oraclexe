...
데이터베이스 객체
    ...
    인덱스 : 테이블의 데이터에 대한 빠른거맥을 지원해주는 색인 객체입니다.
...
-- ROWNUM 예제 : 쿼리 결과의 각 행에 순차적인 번호를 할당해준다.
...
-- 10000이상인 값으로 수정해야 check option 위배 X
-- WITH CHECK OPTION의 조건에 위배 되지 않아 변경 가능하다.
...
/*
시퀀스(Sequence)
    연속적인 숫자 값을 자동으로 증강시켜서 값을 리턴하는 객체입니다.
    채번할 때 많이 사용한다.
*/
...
인덱스 (INDEX)
    ...
    PK - 테이블 생성시 자동으로 인덱스 생성이 된다.
*/

SELECT * FROM employees
WHERE last_name='King';

SELECT last_name, ROWID FROM employees --ROWID : 메모리 주소 위치
ORDER BY last_name;
...
/*
동의어(SYNONYM)
    동의어를 생성하여 객체에 대한 이름을 부여 할 수 있습니다.
