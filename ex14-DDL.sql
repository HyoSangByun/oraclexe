...
/*
데이터 유형
    VARCHAR(size) 가변 길이 문자 데이터 (4000)
...
/*
ALTER 문
    데이터베이스 객체 구조나 속성으 변경할 떄 사용되는 SQL 명령문입니다.
*/
...
ALTER TABLE simple ADD(addr VARCHAR2(3));
-- 컬럼 삭제하기2
ALTER TABLE simple DROP(addr);

DESC simple;

-- DROP 문 : 객체를 삭제할 때 사용되는 SQL 명령문 입니다.
-- 테이블 삭제
DROP TABLE simple;
...
SELECT * FROM user_constraints
WHERE constraint_name = 'SYS_C008364'
;
