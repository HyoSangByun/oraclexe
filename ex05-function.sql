...
함수(Function)
    매개변수를 받아 특정 계산을 수행하고 결과를 반환하는 구조로 되어 있다.
...
SELECT employee_id, last_name, department_id
FROM employees
WHERE last_name = INITCAP('higgins') --LOWER함수는 리소스를 많이 쓰기 때문에 INITCAP이 더 효율적
...
/*
날짜 연산
    날짜에서 숫자를 더하거나 뺴서 결과 날짜 값을 구합니다.
...
SELECT last_name, salary, NVL(commission_pct, 0), (salary * 12) AS Y_SAL, -- null값을 0으로 변환
        (salary * 12) + (salary * 12 * NVL(commission_pct, 0)) AS AN_SAL
FROM employees;

-- NVL2() 함수
-- 첫 번째 표현식을 검사합니다. 첫 번째 표현식이 null이 아니면 두번 째 표현식을 반환합니다.
...
-- COALESCE() 함수
-- 리스트에서 null이 아닌 첫 번째 표현식을 반환합니다. null이 아닐 때 까지 찾아 반환합니다.
